#!/usr/bin/env bash
# Burn sheet → paginated US-Letter PDF (implements PAGE_FORMAT.md).
#
#   Usage: ./build-pdf.sh <sheet.html> <out.pdf>
#
# Copies the sheet to a temp dir, injects print.css (which the screen
# HTML must never carry — PAGE_FORMAT.md scope rule), and paginates via
# Paged.js (pagedjs-cli: Paged.js + headless Chrome). Requires network
# for the sheet's Google Fonts @import.
set -euo pipefail
